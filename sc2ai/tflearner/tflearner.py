...
class Rollout:
    ...
    def add_step(self, state, memory, action=None, reward=None, mask=None, done=None):
        ...
        :param memory: The memory which the action was taken in. Memory is an extension of the state generated
            by the agent.
        ...
        for i in range(self.num_games):
            ...
            if rollout.done:
                feed_dict = {
                    ...
                    **self.agent.get_feed_dict(rollout.states, rollout.memories,
                                               rollout.masks, rollout.actions, rollout.bootstrap_state)
                }
    ...
    def generate_trajectory(self):
        ...
        while True:
            ...
            for i, rollout in enumerate(self.rollouts):
                rollout.add_step(states[i], memory[i], action_indices[i], rewards[i], masks[i], dones[i])
            states = new_states
            masks = new_masks
            if all(dones):
                # Add in the done state for rollouts which just finished for calculating the bootstrap value.
                for i, rollout in enumerate(self.rollouts):
                    rollout.add_step(states[i], memory[i])
                return
